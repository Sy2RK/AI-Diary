...
def _http_json(url: str, headers: Dict[str, str], timeout_sec: int = 20) -> Any:
    # TikHub is behind Cloudflare; the default Python urllib User-Agent can be blocked (e.g. error code 1010).
    # Use a more typical UA by default, while still allowing caller to override.
...
def fetch_user_notes_alt_userid(user_id: str, api_key: str, cursor: str = "", timeout_sec: int = 20) -> Any:
    """
    Compatibility retry: some variants use `userid` instead of `user_id`.
    """
    uid = (user_id or "").strip()
    if not uid:
        raise ValueError("user_id is empty")
...
def _download(url: str, dest: Path, timeout_sec: int = 30) -> Path:
    ...
    if not suffix:
        if ctype in ("image/jpeg", "image/jpg"):
            ...
            # fallback from url
            for ext in [".jpg", ".jpeg", ".png", ".webp", ".gif", ".mp4", ".mov", ".m3u8", ".webm"]:
                if ext in url.lower():
...
def _extract_user_notes_list(resp: Any) -> Tuple[List[Dict[str, Any]], str, bool]:
    ...
    if isinstance(data, dict):
        cursor = str(data.get("cursor") or data.get("next_cursor") or data.get("page_token") or "").strip()
        has_more = bool(data.get("has_more") or data.get("hasMore") or bool(cursor))
        ...
        # variants: data.items / data.list / data.data
        for key in ("items", "list", "data"):
            notes2 = data.get(key)
            if isinstance(notes2, list):
                return [n for n in notes2 if isinstance(n, dict)], cursor, has_more
...
def _save_notes_like_rednotes(
    ...
    qwen_cfg_path: str = "rednotes.yaml",
...
def _qwen_vl_generate_body_text(
    ...
    if not api_key:
        raise RuntimeError("Missing Qwen API key. Set rednotes.yaml qwen.api_key or env QWEN_API_KEY/DASHSCOPE_API_KEY")
...
def main() -> int:
    ...
    src.add_argument("--from-config", action="store_true", help="Read accounts+maxcrawl from rednotes.yaml and crawl all")
    ...
    parser.add_argument("--limit", type=int, default=0, help="Max notes per profile for --profile-url/--from-config (0 uses rednotes.yaml maxcrawl)")
    ...
    parser.add_argument("--qwen-config", default="rednotes.yaml", help="rednotes.yaml path for Qwen config")
    parser.add_argument(
        "--qwen-model",
        default="",
        help="Qwen multimodal model name (default: use rednotes.yaml qwen.model; recommended for video: qwen3-vl-plus)",
    )
    parser.add_argument(
    ...
    if bool(args.from_config):
        ...
        cfg, _raw = load_rednotes_config(str(args.qwen_config))
        ...
        if since_str:
            try:
                ...
                return 2
        if not accounts:
            print(f"[tikhub] no accounts in {args.qwen_config}", file=sys.stderr)
            return 2
        for acc in accounts:
            ...
            if not uid or not name:
                continue
            print(f"[tikhub] profile={name} user_id={uid} limit={per_limit}")
            ...
            for m in metas:
                ...
                if not nid:
                    continue
                notes_to_fetch.append({"note_id": nid, "account_name": name})
        ...
        for m in metas:
            ...
            if not nid:
                continue
            notes_to_fetch.append({"note_id": nid, "account_name": uid})
    ...
    for i, base in enumerate(notes_to_fetch, start=1):
        ...
        try:
            ...
            if str(primary.get("type") or "").strip().lower() == "video":
        ...
        source_url = _extract_share_url(note) or f"https://www.xiaohongshu.com/explore/{nid}"
        tags = _extract_tags(note)
        cover_url = _extract_cover_url(note)
        image_urls = _extract_image_urls_from_note(note)
        video_urls = _extract_video_media_urls(video_note or note)
            ...
            enriched.append(
                {
                    ...
                    "video_urls": [],
                    "publish_time": "",
                    "error": f"note_info:{base.get('_note_info_error')}",
                }
            )
            continue
    ...
    _save_notes_like_rednotes(
        ...
        qwen_cfg_path=str(args.qwen_config),
