...
from scripts.integrations.bitable_client import BitableClient, _to_attachment_value, _to_link_value

FEISHU_OPENAPI = "https://open.feishu.cn/open-apis"
UNIQUE_FIELD_NAME = "唯一键"
IMAGES_ATTACH_FIELD = "配图(附件)"
COVER_ATTACH_FIELD = "封面"
DATE_FIELD = "日期"
SCORE_FIELD = "评分"
SOURCE_URL_FIELD = "原博文链接"
VIDEO_FIELD = "内含视频"
ONE_LINER_ANALYSIS_FIELD = "一句话分析"
STAR_FIELD = "应用推荐度"
REVIEW_NOTE_FIELD = "备注（用于人工后期审阅时批注）"
REVIEWER_FIELD = "审阅人（填写姓名）"
RELATED_PROJECT_FIELD = "相关项目（用于备注可用项目）"
...
def _load_one_liner_prompt(config_path: str) -> str:
    ...
    if not prompt:
        raise RuntimeError("Missing prompt_bitable_one_liner in config")
    return prompt


def _load_one_liner_analysis_prompt(config_path: str) -> str:
    """
    Prompt for "一句话分析" (separate from "一句话总结").

    Can be overridden via config: prompt_bitable_one_liner_analysis
    """
    from crawler_rednotes.config_rednotes import load_rednotes_config

    _cfg, raw = load_rednotes_config(config_path)
    prompt = str(raw.get("prompt_bitable_one_liner_analysis") or "").strip()
    if prompt:
        return prompt

    return (
        "你是一位服务于面向海外市场的移动端游戏公司的内容分析师，目标是把长分析压缩成“可直接阅读的一句话”。\n\n"
        "输入：一个 JSON 数组，每个元素包含：\n"
        "- key：唯一键\n"
        "- title：标题\n"
        "- summary：已有摘要/二次分析（可能较长）\n\n"
        "输出要求（必须严格遵守）：\n"
        "- 只输出 1 个 JSON 数组，数组长度与输入一致\n"
        "- 每个元素必须是：{\"key\": \"...\", \"one_liner\": \"...\"}\n"
        "- one_liner：中文 24–60 字，1 句话，不要换行，不要使用 URL，不要套话，不要列点\n"
        "- 必须包含两部分：①核心要点总结；②对公司业务的潜在价值（提效/迁移/市场等）\n"
    )
...
def _load_qwen_env_from_rednotes(config_path: str) -> Dict[str, Any]:
    ...
    if not cfg.qwen or not cfg.qwen.api_key:
        raise RuntimeError("Missing qwen.api_key in config")
    return {
...
def _generate_one_liners_qwen(
    ...
    return _generate_one_liners_qwen_with_prompt(prompt_tpl, config_path, items, model=model)


def _generate_one_liners_qwen_with_prompt(
    prompt_tpl: str,
    config_path: str,
    items: List[Dict[str, Any]],
    model: str = "qwen3-max",
...
def _record_to_bitable_fields(rec: UnifiedRecord) -> Dict[str, Any]:
    ...
    return {
        ...
        ONE_LINER_ANALYSIS_FIELD: None,  # filled later
        DATE_FIELD: None,  # filled later
        SCORE_FIELD: None,  # filled later
        SOURCE_URL_FIELD: None,  # filled later (Link field expects object)
        VIDEO_FIELD: None,  # filled later
        COVER_ATTACH_FIELD: None,  # filled later (attachment)
        # Attachment fields (for XHS). Filled later.
        IMAGES_ATTACH_FIELD: None,
        # Extra fields reserved for manual review (left blank by script).
        STAR_FIELD: None,
        REVIEW_NOTE_FIELD: None,
        REVIEWER_FIELD: None,
        RELATED_PROJECT_FIELD: None,
    }
...
_VIEW_FIELD_SPECS: Dict[str, Dict[str, Any]] = {
    ...
    "标题": {"type": 1},
    "一句话总结": {"type": 1},
    ONE_LINER_ANALYSIS_FIELD: {"type": 1},
    # Bitable attachment field type is 17
    COVER_ATTACH_FIELD: {"type": 17},
    DATE_FIELD: {"type": 1},
    SCORE_FIELD: {"type": 1},
    SOURCE_URL_FIELD: {"type": 15},
    VIDEO_FIELD: {"type": 7},
    IMAGES_ATTACH_FIELD: {"type": 17},
    # Feishu OpenAPI doesn't expose a dedicated "rating" field type; use Number and format in UI if needed.
    STAR_FIELD: {"type": 2, "property": {"formatter": "0"}},
    REVIEW_NOTE_FIELD: {"type": 1},
    REVIEWER_FIELD: {"type": 1},
    RELATED_PROJECT_FIELD: {"type": 1},
}
...
def _load_bitable_config(config_path: str) -> Tuple[str, str, str, str, bool, str]:
    ...
    if not cfg.feishu or not cfg.feishu.app_id or not cfg.feishu.app_secret:
        raise RuntimeError("Missing feishu.app_id/app_secret in config (required for Bitable OpenAPI)")
    if not cfg.bitable or not cfg.bitable.app_token:
        raise RuntimeError("Missing bitable.app_token in config")
...
def load_xhs_records(run_dir: str = "", config_path: str = "configs/legacy/rednotes.yaml", score_threshold: float = 6.0) -> List[UnifiedRecord]:
        ...
        if isinstance(items, list):
            for it2 in items:
                ...
                saved_paths_by_id[nid] = {
                    ...
                    "video_paths": it2.get("video_paths") or [],
                }
    ...
    for it in stage1:
        ...
        # Prefer generated cover under <run_root>/covers/<note_id>.* when available.
        gen_cover = ""
        for ext in (".png", ".jpg", ".jpeg", ".webp"):
            p2 = run_root / "covers" / f"{note_id}{ext}"
            if p2.exists():
                gen_cover = str(p2)
                break
        cover = gen_cover or cover_url or cover_path
        ...
        local_videos = saved_paths_by_id.get(note_id, {}).get("video_paths")
        has_video = False
        if isinstance(local_videos, list) and any(str(x).strip() for x in local_videos):
            has_video = True
        else:
            video_urls = raw_note.get("video_urls")
            if isinstance(video_urls, list) and any(str(x).strip() for x in video_urls):
                has_video = True
            elif str(raw_note.get("video_url") or "").strip():
                has_video = True
            elif str(raw_note.get("type") or "").strip().lower() in ("video", "note_video"):
                has_video = True
...
def load_wechat_records(
    ...
    for it in stage2:
        ...
        cover = ""
        run_root = stage2_path.parent
        for ext in (".png", ".jpg", ".jpeg", ".webp"):
            p2 = run_root / "covers" / f"{unique_key}{ext}"
            if p2.exists():
                cover = str(p2)
                break
...
def sync_to_bitable(
    ...
    config_path: str = "configs/legacy/rednotes.yaml",
    ...
    def upload_if_local(path: str) -> str:
        ...
        try:
            ...
            if fp.exists() and fp.is_file():
                # Skip non-image artifacts (e.g. .bin from heif downloads).
                ext = fp.suffix.lower()
                if ext == ".bin":
                    return ""
    ...
    try:
        analysis_prompt = _load_one_liner_analysis_prompt(config_path)
        analysis_one_liners = _generate_one_liners_qwen_with_prompt(analysis_prompt, config_path, items_for_llm, model="qwen3-max")
    except Exception as exc:  # noqa: BLE001
        print(f"[bitable] one_liner_analysis skipped/failed: {exc}")
        analysis_one_liners = {}
    ...
    def filtered_fields(r: UnifiedRecord) -> Dict[str, Any]:
        ...
        f[DATE_FIELD] = date_ymd or None
        link = _to_link_value(r.url, text=(r.title or "").strip())
        if link is not None:
            f[SOURCE_URL_FIELD] = link

        cover_ft = upload_if_local(r.cover_url)
        if cover_ft:
            f[COVER_ATTACH_FIELD] = _to_attachment_value([cover_ft])

        if r.score is not None:
            # Feishu Bitable text/multiline fields require string values; keep stable formatting.
            f[SCORE_FIELD] = f"{float(r.score):.1f}"
            # Map AI score (6~10) to 1~5 stars for the rating field.
            # Since records are already filtered by min_score>=6, treat 6 as baseline (1 star).
            s = float(r.score)
            s = max(6.0, min(10.0, s))
            stars = int(round(s - 5.0))  # 6->1, 7->2, 8->3, 9->4, 10->5
            stars = max(1, min(5, stars))
            f[STAR_FIELD] = stars
        # Checkbox field expects boolean
        f[VIDEO_FIELD] = bool(r.has_video)
        ol = one_liners.get(r.unique_key, "").strip()
        if ol:
            f["一句话总结"] = ol
        aol = analysis_one_liners.get(r.unique_key, "").strip()
        if aol:
            f[ONE_LINER_ANALYSIS_FIELD] = aol
        ...
        if r.source == "xhs":
            ...
            for ip in (r.image_urls or []):
                ...
                if ft:
                    image_tokens.append(ft)
            imgs = _to_attachment_value(image_tokens)
            if imgs is not None:
                f[IMAGES_ATTACH_FIELD] = imgs
...
def _ensure_source_tables(config_path: str, token: str, app_token: str) -> Dict[str, str]:
    """
    Ensure per-source tables exist and persist table_id back to config.

    Uses `config: bitable.tables` (nested mapping) when available.
    """
    from crawler_rednotes.config_rednotes import load_rednotes_config
    ...
    cfg, _raw = load_rednotes_config(config_path)
    if not cfg.bitable:
        raise RuntimeError("Missing bitable config in config")
...
def main() -> int:
    ...
    p_init.add_argument("-c", "--config", default=DEFAULT_PROJECT_CONFIG, help="配置入口（统一配置或 legacy rednotes.yaml）")
    p_init.add_argument("--profile", default="", help="配置环境(profile)，仅统一配置生效")
    ...
    p_xhs.add_argument("-c", "--config", default=DEFAULT_PROJECT_CONFIG, help="配置入口（统一配置或 legacy rednotes.yaml）")
    ...
    p_wc.add_argument("-c", "--config", default=DEFAULT_PROJECT_CONFIG, help="配置入口（统一配置或 legacy rednotes.yaml）")
    ...
    p_all.add_argument("-c", "--config", default=DEFAULT_PROJECT_CONFIG, help="配置入口（统一配置或 legacy rednotes.yaml）")
    p_all.add_argument("--profile", default="", help="配置环境(profile)，仅统一配置生效")
    ...
    try:
        cfg_path = resolve_legacy_config_for_cli(args.config, kind="rednotes", profile=getattr(args, "profile", ""))
        if args.cmd == "init":
            app_id, app_secret, app_token, default_table_id, use_views, _write_mode = _load_bitable_config(cfg_path)
            token = _get_tenant_access_token(app_id, app_secret)
            ...
            # Remove legacy/deprecated columns only; do NOT delete current attachment fields.
            # Remove legacy/deprecated columns (field renames, old layouts).
            remove_names = {
                "编号",
                "封面",
                "全部图片内容",
            ...
            if use_views:
                ...
                try:
                    ...
                    for it in existing:
                        ...
                        if name == COVER_ATTACH_FIELD and int(it.get("type") or 0) == 17:
                            continue
            ...
            table_ids = _ensure_source_tables(cfg_path, token=token, app_token=app_token)
        ...
        if args.cmd == "xhs":
            records = load_xhs_records(run_dir=args.run_dir, config_path=cfg_path, score_threshold=args.min_score)
            if args.note_id:
        ...
        if args.cmd == "all":
            ...
            _app_id, _app_secret, _app_token, _table_id, use_views, write_mode = _load_bitable_config(cfg_path)
            cleared_once = False

            try:
                xhs = load_xhs_records(run_dir=args.xhs_run_dir, config_path=cfg_path, score_threshold=args.xhs_min_score)
                c, u = sync_to_bitable(
                    xhs,
                    config_path=cfg_path,
                    ensure_fields=not args.no_ensure_fields,
