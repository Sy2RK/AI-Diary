...
def load_cover_gen_config(path: str = "configs/legacy/cover_gen.yaml") -> CoverGenConfig:
    raw = _read_yaml(path)
    ...
    if not prompt_template:
        raise RuntimeError("Missing prompt.template in cover config")
...
def _load_dashscope_key_from_project_config(config_path: str) -> str:
    raw = _read_yaml(config_path)

    # Legacy rednotes config: qwen.api_key
    qwen_raw = raw.get("qwen") if isinstance(raw.get("qwen"), dict) else {}
    key = str(qwen_raw.get("api_key") or "").strip()
    if key:
        return key

    # Unified config: xhs.qwen.api_key
    xhs_raw = raw.get("xhs") if isinstance(raw.get("xhs"), dict) else {}
    qwen2_raw = xhs_raw.get("qwen") if isinstance(xhs_raw.get("qwen"), dict) else {}
    return str(qwen2_raw.get("api_key") or "").strip()


def _inject_wanx_api_key_fallback(cfg: CoverGenConfig, config_path: str, *, verbose: bool = False) -> None:
    if (cfg.wanx.api_key or "").strip():
        return
    fallback = _load_dashscope_key_from_project_config(config_path)
    if not fallback:
        return
    cfg.wanx.api_key = fallback
    if verbose:
        print("[cover] cover.wanx.api_key is empty, fallback to qwen.api_key from project config")
...
def wanx_generate_one(
    ...
    *,
    ...
    if not api_key:
        raise RuntimeError("Missing DashScope API key (set cover.wanx.api_key in config or env DASHSCOPE_API_KEY)")
...
def _load_one_liners(config_path: str, items: List[Dict[str, Any]]) -> Dict[str, str]:
    from scripts.integrations.bitable_sync import _generate_one_liners_qwen
...
def generate_covers_xhs(
    ...
    *,
    ...
    from scripts.integrations.bitable_sync import load_xhs_records

    cfg = load_cover_gen_config(cover_cfg_path)
    _inject_wanx_api_key_fallback(cfg, config_path, verbose=verbose)
...
def generate_covers_wechat(
    ...
    *,
    ...
    from scripts.integrations.bitable_sync import load_wechat_records
...
def main() -> int:
    ...
    p_xhs.add_argument(
        "--cover-config",
        default="",
        help="封面配置入口（留空跟随 --config；可传 unified 或 legacy cover_gen.yaml）",
    )
    ...
    try:
        resolved_project_cfg = resolve_legacy_config_for_cli(args.config, kind="rednotes", profile=args.profile)
        cover_entry = str(args.cover_config or args.config).strip()
        resolved_cover_cfg = resolve_legacy_config_for_cli(cover_entry, kind="cover_gen", profile=args.profile)
    except Exception as exc:  # noqa: BLE001
        print(f"[cover] config resolve failed: {exc}")
        return 2
    ...
    if args.cmd == "xhs":
        ...
        n = generate_covers_xhs(
            run_dir=run_dir,
            config_path=resolved_project_cfg,
            cover_cfg_path=resolved_cover_cfg,
