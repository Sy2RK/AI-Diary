...
def stage1_analyze_run(
    ...
    if not prompt:
        raise RuntimeError("prompt_stage1_xhs is empty in rednotes.yaml")
...
def stage2_summarize_run(run_dir: Path, cfg_path: str) -> Path:
    ...
    if not prompt:
        raise RuntimeError("prompt_stage2_xhs is empty in rednotes.yaml")
    api_key, base_url, model, timeout_sec, enable_thinking = _resolve_qwen_settings(cfg)
    client = QwenClient(api_key=api_key, base_url=base_url, model=model, timeout_sec=timeout_sec)
...
def main() -> int:
    ...
    parser.add_argument("-c", "--config", default="rednotes.yaml", help="rednotes.yaml path")
    ...
    try:
        if not args.stage2_only:
            stage1_path = stage1_analyze_run(run_dir, args.config, limit=args.limit)
            print(f"Saved stage1 to {stage1_path}")
        stage2_path = stage2_summarize_run(run_dir, args.config)
